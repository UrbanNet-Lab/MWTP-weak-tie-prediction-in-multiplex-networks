...
from .MultiLayer import SemanticAttention, BitwiseMultipyLogis
...
class SupervisedGraphSage(nn.Module):
    ...
    def __init__(self, enc, embed_dim, layer_num, device, inter_aggregation):
        ...
        if inter_aggregation == 'semantic':
            self.layerNodeAttention_weight = SemanticAttention(self.layer_num,self.embed_dim,device, dropout=0.5, alpha=0.5).to(device)
        elif inter_aggregation == 'logit':
            self.layerNodeAttention_weight = BitwiseMultipyLogis(self.layer_num,self.embed_dim,device).to(device)
